...
public class LongestIncreasingSubsequence {
	...
	public int lengthOfLIS(int[] nums) {
		 ...
		 * by the sort(int [], int, int) method) prior to making this call. If
		 * it is not sorted, the results are undefined. If the range contains
		 * multiple elements with the specified value, there is no guarantee
		 * which one will be found.
		 * 
		 * Parameters: a the array to be searched fromIndex the index of the
		 * first element (inclusive) to be searched toIndex the index of the
		 * last element (exclusive) to be searched key the value to be searched
		 * for Returns: index of the search key, if it is contained in the array
		 * within the specified range; otherwise, (-(insertion point) - 1). The
		 ...
		 * greater than the key, or toIndex if all elements in the range are
		 * less than the specified key. Note that this guarantees that the
		 * return value will be >= 0 if and only if the key is found. Throws:
		 * IllegalArgumentException - if fromIndex > toIndex
		 * ArrayIndexOutOfBoundsException - if fromIndex < 0 or toIndex >
		 * a.length Since: 1.6
		 */
		for (int x : nums) {
